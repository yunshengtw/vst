...
void shashtbl_init(void)
{
    for (UINT32 bank = 0; bank < NUM_BANKS; bank++) {
        ...
        g_shashtbl[bank].free_node_pool = 0;
    }
    mem_set_dram(HASH_NODE_ADDR, 0, HASH_NODE_BYTES);
    mem_set_dram(HASH_BUCKET_ADDR, 0, HASH_BUCKET_BYTES);
}
void shashtbl_insert(UINT32 const bank, UINT32 const key, UINT32 const data)
{
    ...
    if (g_shashtbl[bank].free_node_pool == 0) {
    ...
    #ifdef VST
    ASSERT((UINT64)new_node > DRAM_BASE);
    #else
	...
	while(node) {
		if(read_dram_32(node) == key) {
			if(prevnode) {
                ...
                write_dram_32(HASH_BUCKET_ADDR + ((bank * HASH_BUCKET_SIZE + hash_val) * sizeof(hashnode_ptr)), tnode);
            }

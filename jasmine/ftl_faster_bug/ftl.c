...
/* 	if (check_format_mark() == FALSE) { */
    ...
    mem_set_sram(g_mem_to_clr, 0, PAGES_PER_BLK / 8);
...
static void format(void)
{
    ...
    mem_set_dram(FREE_BMT_ADDR, 0, FREE_BMT_BYTES);
    // log page mapping table (linear) - NOT USED
/*     mem_set_dram(LOG_PMT_ADDR, INVALID, LOG_PMT_BYTES); */
    // validation check bitmap table
    mem_set_dram(VC_BITMAP_ADDR, 0, VC_BITMAP_BYTES);
    // second chance bitmap table
    mem_set_dram(SC_BITMAP_ADDR, 0, SC_BITMAP_BYTES);
    // block wearout table
    mem_set_dram(BLK_ERASE_CNT_ADDR, 0, BLK_ERASE_CNT_BYTES);
...
static void init_metadata_sram(void)
{
    for (UINT32 bank = 0; bank < NUM_BANKS; bank++) {
        ...
        mem_set_sram(&g_ftl_statistics[bank], 0, sizeof(ftl_statistics));
